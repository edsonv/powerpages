    ...
    public function get() {
        ...
        while($row = $result->fetch_object()):
            ...
            if(!empty($row->location_url)) {
                $location_url = '<img src="https://www.google.com/s2/favicons?domain=' . $row->location_url . '" class="img-fluid mr-2" />';
                $location_url .= '<small><a href="' . $row->location_url . '" target="_blank">' . $this->language->admin_links->display->location_url . '</a></small>';
            } else {
                $location_url = '<img src="https://www.google.com/s2/favicons?domain=' . url($row->url) . '" class="img-fluid mr-2" />';
            ...
            /* Clicks */
            $row->clicks = '<i class="fa fa-chart-bar"></i> ' . nr($row->clicks);

            /* Is Enabled Status badge */
            $row->is_enabled = $row->is_enabled ? '<span class="badge badge-pill badge-success"><i class="fa fa-check"></i> ' . $this->language->global->active . '</span>' : '<span class="badge badge-pill badge-warning"><i class="fa fa-eye-slash"></i> ' . $this->language->global->disabled . '</span>';
