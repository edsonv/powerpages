    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(empty($_SESSION['error'])) {
                ...
                $stmt = Database::$database->prepare("UPDATE `users` SET `email` = ?, `name` = ?, `timezone` = ? WHERE `user_id` = {$this->user->user_id}");
                $stmt->bind_param('sss', $_POST['email'], $_POST['name'], $_POST['timezone']);
                $stmt->execute();
                $stmt->close();
