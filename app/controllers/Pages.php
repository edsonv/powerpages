...
class Pages extends Controller {
    ...
    public function index() {
        ...
        if($pages_category_url) {
            ...
            $pages_result = $this->database->query("SELECT `url`, `title`, `description`, `total_views` FROM `pages` WHERE `pages_category_id` = {$pages_category->pages_category_id} ORDER BY `total_views` DESC");
            ...
            $popular_pages_result = $this->database->query("SELECT `url`, `title`, `description`, `total_views` FROM `pages` ORDER BY `total_views` DESC LIMIT 6");
